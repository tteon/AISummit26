...
const armNames = {physical_only:"물리 스키마만", compiled_fibo:"전체 FIBO 매핑", retrieved_fibo:"검색한 FIBO 매핑", direct_single:"단일 에이전트", multi_typed:"역할 분리 · typed", multi_envelope:"역할 분리 · envelope"};
...
function drawRun(data,params) {
  ...
  function filter() {
    ...
    const paired=rows.filter(s=>arms.length>1 && arms.every(a=>counts.get(pairKey(s)).get(a)===1));
    ...
    }).join("")}</tbody></table></div><p class="table-caption">모든 arm에 동일 요청·반복이 정확히 한 건씩 있는 표본만 비교합니다. ${rows.length-paired.length}건은 누락·중복 또는 비교 조건 부족으로 비교에서 제외하고 아래에 보존합니다. 오류 표본은 포함하며 미기록 비용을 0으로 채우지 않습니다. 기존 측정값의 기술 통계입니다.</p>`;
...
async function showEpisode(key,id) {
  ...
  try {
    ...
    const params=executions[0]?.params;
    const actualRows=executions.map(e=>({phase:e.phase, row_count:e.rows, completeness:e.completeness}));
